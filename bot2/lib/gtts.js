...
function tokenize(text) {
  var text_parts = [];
  if (!text)
    throw new Error('Nenhum texto para falar');
  ...
  var server = http.createServer(function(req, res) {
    ...
    if (queryData && queryData.text) {
      ...
      res.end(JSON.stringify({
        code: -1,
        message: `Faltando texto, Tente novamente: ${req.headers.host}?text=your+text`
      }))
    }
  });

  server.listen(port);
  console.log("Servidor de Text-para-Fala em execução " + port);
}
